...
public class Client {
    ...
    private class Sender extends Thread {
        ...
        private void senderLoop(){
            while(sock.isConnected()){
                try{
                    /*
                     * Taking turns protocol, pass on token. isBusy checks whether the line is busy or free
                     * the takingTurns will be sent with the data, so that the next node knows it can send
                     * if there is nothing to send, send a DATA_SHORT packet passing the token
                     * takingTurns and this.id set to 1 is just that it doesn't give errors
                     */

                    //if (takingTurns == id && !isBusy) {
                    Message msg = sendingQueue.take();
                    System.out.println(msg.getType());
                    if( msg.getType() == MessageType.DATA || msg.getType() == MessageType.DATA_SHORT ){
                        takingTurns = (int) (msg.getData().get(1) >> 3) % 4;
    ...
    public void setTakingTurns(int takingTurns) throws InterruptedException {
